  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Card
        ...
        style={{ background: `linear-gradient(135deg, ${culture.primaryColor}22, ${culture.accentColor}33, #F3EFE9)` }}
      >
        <div className="p-8 md:p-12 flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-lg italic font-serif mb-4" style={{ color: '#5A6F5E' }}>{culture.tagline}</p>
          ...
          <div className="space-y-4">
            ...
            {recipes.map((recipe) => (
              <Link key={recipe.id} href={`/culture-kitchen/recipes/${recipe.id}`}>
                <Card className="flex items-center gap-5 p-5 group hover:shadow-md transition-all">
                  <div className="w-16 h-16 rounded-xl flex items-center justify-center text-4xl shrink-0" style={{ backgroundColor: '#F3EFE9' }}>
                    {recipe.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-semibold" style={{ color: '#3B4B3F' }}>{recipe.name}</span>
                  ...
                  <span className="text-sm font-medium shrink-0 group-hover:translate-x-1 transition-transform" style={{ color: '#5A6F5E' }}>→</span>
                </Card>
              </Link>
            ))}
            {recipes.length === 0 && (
          ...
          <div className="space-y-6">
            ...
            {recipes.map((recipe) => (
              <Card key={recipe.id}>
                ...
                <CardContent>
                  <p className="text-sm leading-relaxed mb-3" style={{ color: '#6B6060' }}>{recipe.culturalHistory}</p>
              ...
              <CardContent>
                <div className="space-y-3">
                  {culture.nutritionThemes.map((theme, i) => (
                    <div key={i} className="flex items-start gap-3 p-4 rounded-xl" style={{ backgroundColor: '#F3EFE9' }}>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-5 gap-2 mb-3">
                      {[
                        ...
                        <div key={fact.label} className="text-center p-2 rounded-lg" style={{ backgroundColor: '#F3EFE9' }}>
